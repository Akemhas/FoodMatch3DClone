...
public class ItemSlot : MonoBehaviour
{
    ...
    public void SlotTheItem(Item item)
    {
        ...
        item.transform.DOScale(item.defaultScale * 0.4f, .3f).SetId(itemInstanceID).SetLink(item.gameObject);
        ...
        item.transform.DOMove(transform.position + Vector3.up * .2f, .3f).SetLink(item.gameObject);
    }
